    ...
    <body>
    	...
    	<div class="list" style="margin-bottom: 10px; margin-left: 3%;">
    		...
    		<tr>
	    		...
	    		<span>시작일: <input type="date" name="budgetstart" value="${item.budgetstart}"/><br/>종료일: <input type="date" name="budgetend" value="${item.budgetend}"/></span>
	    		</td>
	    		<td>
	    		예산액:
    		...
    		<tr>
    			...
    			<td><input type="date" name="reg_date" id="todaycheck" value="${detail.reg_date}"></td>
    		</tr>
    		<tr>
    			<th style="text-align:center;">구분</th>
    			<td> 
    				<input type="radio" name="part" value="수입" <c:if test="${detail.part eq '수입'}">checked</c:if> />수입
         			<input type="radio" name="part" value="지출" <c:if test="${detail.part eq '지출'}">checked</c:if> />지출
         		</td>
			</tr>
			<tr> 
				<th style="text-align:center;">결제수단</th>
				<td>
					<input type="radio" name="way" value="현금" <c:if test="${detail.way eq '현금'}">checked</c:if> />현금
	            	<input type="radio" name="way" value="은행" <c:if test="${detail.way eq '은행'}">checked</c:if>/> 은행
	            	<input type="radio" name="way" value="카드" <c:if test="${detail.way eq '카드'}">checked</c:if>/> 카드
	            </td>
            </tr>
			<tr>
    			...
    			<td>
    				<select name="kind">
			            <option value="식비" <c:if test="${detail.kind eq '식비'}">selected</c:if>>식비</option>
			            <option value="회비" <c:if test="${detail.kind eq '회비'}">selected</c:if>>회비</option>
			            <option value="교통" <c:if test="${detail.kind eq '교통'}">selected</c:if>>교통</option>
			            <option value="통신" <c:if test="${detail.kind eq '통신'}">selected</c:if>>통신</option>
			            <option value="문화" <c:if test="${detail.kind eq '문화'}">selected</c:if>>문화</option>
			            <option value="의류" <c:if test="${detail.kind eq '의류'}">selected</c:if>>의류</option>
			            <option value="수입" <c:if test="${detail.kind eq '수입'}">selected</c:if>>수입</option>
			            <option value="기타" <c:if test="${detail.kind eq '기타'}">selected</c:if>>기타</option>
	            	</select>
    			</td>
    		</tr>
			<tr>
	            ...
	            <td>
	            	<input type="text" name="content" style="width:135px;" value="${detail.content}"/>
	        	</td>
        	</tr>
        	<tr>
        		<th style="text-align:center;">금액</th>	
        		<td> 
        			<input type="text" name="price" style="text-align:center; width:135px;" value="${detail.money }"/>
        		</td>
        	</tr>
        	<tr>
        		<th style="text-align: center;"><input id="resister" type="button" value ="가계부수정" onclick="budgetSubmit(4)"/></th>
        		<th style="text-align: center;"><input id="resister" type="button" value ="수정취소" onclick="budgetSubmit(5)"/></th>
        	</tr>
        	</table>
        	</form>
        </div>
    ...
    <script>
		...
		*/
		...
		function budgetSubmit(index){
			...
			if(index==4){
				document.myForm.action="cashupdate";
			}
			if(index==5){
				document.myForm.action='mainlist';
			}
